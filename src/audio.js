/**
 * Shared audio adapters — edge-tts (TTS) and whisper-cli (STT).
 * Self-contained, no project-specific dependencies.
 *
 * ENV:
 *  EDGE_TTS_BIN      – path to edge-tts binary   (default: 'edge-tts')
 *  EDGE_TTS_VOICE    – default TTS voice          (default: 'pl-PL-MarekNeural')
 *  EDGE_TTS_RATE     – speech rate modifier       (default: '-5%')
...
const WHISPER_CLI   = process.env.WHISPER_CLI     || "/home/bpast/whisper.cpp/build/bin/whisper-cli";
const WHISPER_MODEL = process.env.WHISPER_MODEL_BIN || "";

const KNOWN_MODEL_PATHS = [
  "/home/bpast/whisper.cpp/models/ggml-base.bin",
  "/home/bpast/whisper.cpp/models/ggml-base.en.bin",
 ...
 * @param {string} [voice]     - Edge-TTS voice name (default: EDGE_TTS_VOICE)
 * @param {string} [speechRate] - Rate modifier, e.g. "+10%" or "-5%"
...
export const synthesizeWithEdgeTts = async (text, voice = "", speechRate = "") => {
  ...
  // Three strategies (matching PHP EdgeTtsAdapter):
  const strategies = [
      ...
      try {
        ...
        if (await fileExists(mp3Path)) {
          ...
          if (size >= 100) {
            ...
            console.log(`[edge-tts] ${size} bytes, voice=${usedVoice}`);
            return { mp3Buffer, backend: "edge-tts", voice: usedVoice, size };
          }
        }
      } catch (err) {
        lastError = err;
        console.warn(`[edge-tts] strategy failed: ${err.message}`);
      }
...
export const transcribeWithWhisperCli = async (audioBuffer, language = "pl", mimeType = "audio/mpeg") => {
  ...
  console.log(`[whisper-cli] language=${language}, model=${modelPath || "default"}`);
  ...
  try {
    ...
    if (await fileExists(outTxt)) {
      ...
      console.log(`[whisper-cli] ${text.length} chars`);
    ...
    console.log(`[whisper-cli] (stdout) ${text.length} chars`);
    return { text, backend: "whisper-cli", language };
  } finally {
